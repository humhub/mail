...
<div class="btn-group">
    ...
    <ul id="dropdown-messages" class="dropdown-menu"></ul>
</div>
...
<script>
    ...
    setInterval(function () {
        jQuery.getJSON("<?= Url::to(['/mail/mail/get-new-message-count-json']); ?>", function (json) {
            setMailMessageCount(parseInt(json.newMessages));
        });
    }, reloadMessageCountInterval);

    setMailMessageCount(<?= $newMailMessageCount; ?>);
    ...
    $('#icon-messages').click(function () {
        ...
        $('#dropdown-messages').append('<li class="dropdown-header"><div class="arrow"></div><?= Yii::t('MailModule.widgets_views_mailNotification', 'Messages'); ?> <?= Html::a(Yii::t('MailModule.widgets_views_mailNotification', 'New message'), Url::to(['/mail/mail/create', 'ajax' => 1]), array('class' => 'btn btn-info btn-xs', 'id' => 'create-message-button', 'data-target' => '#globalModal')); ?></li> <ul class="media-list"><li id="loader_messages"><div class="loader"><div class="sk-spinner sk-spinner-three-bounce"><div class="sk-bounce1"></div><div class="sk-bounce2"></div><div class="sk-bounce3"></div></div></div></li></ul><li><div class="dropdown-footer"><a class="btn btn-default col-md-12" href="<?= Url::to(['/mail/mail/index']); ?>"><?= Yii::t('MailModule.widgets_views_mailNotification', 'Show all messages'); ?></a></div></li>');

        $.ajax({
            'type': 'GET',
            'url': '<?= Url::to(['/mail/mail/notification-list']); ?>',
            'cache': false,
