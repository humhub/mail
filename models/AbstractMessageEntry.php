...
abstract class AbstractMessageEntry extends ActiveRecord
{
    ...
    /**
     * Invalidates the cached new-message count of every participant of this entry's conversation.
     */
    private function invalidateParticipantsNewMessageCountCache(): void
    {
        $userIds = UserMessage::find()
            ->select('user_id')
            ->where(['message_id' => $this->message_id])
            ->column();

        foreach ($userIds as $userId) {
            UserMessage::invalidateNewMessageCountCache($userId);
        }
    }
